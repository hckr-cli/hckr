...
class AzureConnectionTester:
    ...
    def test_connection(
        ...
        """
        result = {"success": False, "message": "", "details": {}, "errors": []}
        ...
        try:
            ...
            try:
                ...
                result["message"] = (
                    "Azure SDK packages are required. Install with: pip install azure-identity azure-mgmt-resource"
                )
                return False, result
            ...
            if not all([credentials.tenant_id, credentials.client_id]):
                result["errors"].append(
                    "Missing required credentials: tenant_id and client_id are required"
                )
                ...
                try:
                    ...
                    if credentials.resource_group:
                        ...
                        try:
                            ...
                            result["errors"].append(
                                f"Resource group access failed: {str(e)}"
                            )

                    self.logger.info(
                        f"Successfully connected to subscription: {credentials.subscription_id}"
                    )

                except Exception as e:
                    result["errors"].append(
                        f"Resource management access failed: {str(e)}"
                    )
